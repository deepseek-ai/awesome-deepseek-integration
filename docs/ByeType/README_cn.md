...
- 🖼️ **图像文字提取** —— 通过自定义快捷键截图选区，AI 识别文字后复制到剪贴板。能智能合并终端/PDF 截图中因窗口宽度产生的硬换行，还能还原被行号、分屏切碎的终端代码
- 📝 **Markdown 驱动提示词** —— 词汇、格式化规则、输出风格都是可编辑的 Markdown 文件。把不同风格（口语化、正式书写、翻译、邮件润色等）绑定到你自己的快捷键，一键切换
