...
<td> BEl asistente de IA admite conexión directa a la interfaz DeepSeek-V3.1 con el potente modelo de código Claude Code, permitiendo su uso sin intermediarios de red (reducción de costes del 90%). Soporta lectura de imágenes y archivos PDF con generación gratuita de imágenes; permite integración personalizada con otros modelos de IA; habilita búsqueda en tiempo real en la web mediante navegador local. Además, ofrece opciones gratuitas como el modelo R1.
    ...
    <tr>
        ...
        <td>MateClaw es un sistema operativo personal de IA de código abierto construido sobre Spring Boot + Spring AI Alibaba, con soporte integrado para DeepSeek V4 Pro y V4 Flash. Un único JAR despliega el mismo agente en la consola web, la aplicación de escritorio, el widget incrustable, el SDK de plugins Java y 8 canales de mensajería (DingTalk, Feishu, WeChat Work, WeChat, Telegram, Discord, QQ, Slack), con conmutación por error multi-proveedor, agentes ReAct + Plan-and-Execute, un LLM Wiki con citas, ciclo de vida de memoria, MCP y Tool Guard.</td>
    </tr>
</table>
