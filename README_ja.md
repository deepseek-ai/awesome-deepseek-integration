...
<tr>
   ...
   <tr>
    ...
    <tr>
        ...
        <td><a href="https://github.com/jiqi136/Ai-Assistant">小説執筆＆AIプログラミングアシスタント</a></td>
        <td> 【ソースコード公開】小説執筆＆AIプログラミングアシスタントは、以下のコアアドバンテージを備えています。
        モデル集約：最新のDeepSeekをネイティブサポートし、Gemini、Claude、GPT、通義千問（Tongyi Qianwen）、Kimiなど国内外の最先端モデルにもシームレスに接続可能。ワンストップのインテリジェント体験を実現します。
        マルチモーダル対応：画像やPDFの内容を高精度に認識し、先進モデルを活用した高品質なテキストからの画像生成にも対応します。
        セキュリティと管理：会話履歴はローカルで暗号化保存（SQLite）され、便利な履歴検索・お気に入り登録・分類機能を提供します。
        精密なコンテキスト制御：個々の会話を自由に編集・削除でき、コンテキストを正確にコントロールすることでAIのパフォーマンスを最大限に引き出します。
        ネイティブでスムーズな操作感：自社開発の軽量GUIフレームワークをベースに、AIとの対話に特化した設計。タイプライターのような流れるストリーミング出力で快適な使用感を実現します。
        </td>
	</tr>
    <tr>
